...
CREATE TABLE `aps_decade_plan` (
  ...
  `specification` varchar(50) DEFAULT NULL COMMENT '规格（长嘴/短嘴等）',
  `quantity_total` int NOT NULL COMMENT '投料总量（箱）',
  `final_quantity` int NOT NULL COMMENT '成品数量（箱）',
  `production_unit` varchar(50) DEFAULT NULL COMMENT '生产单元',
  ...
  `production_date_range` varchar(100) DEFAULT NULL COMMENT '成品生产日期范围',
  ...
  KEY `idx_work_order` (`work_order_nr`),
  KEY `idx_planned_time` (`planned_start`,`planned_end`),
  KEY `idx_validation_status` (`validation_status`),
  KEY `fk_decade_plan_material` (`article_nr`),
...
CREATE TABLE `aps_feeding_order` (
  ...
  `feeder_code` varchar(20) NOT NULL COMMENT '喂丝机代码',
  `feeder_type` varchar(50) DEFAULT NULL COMMENT '喂丝机型号',
  ...
  `feeding_speed` decimal(10,2) DEFAULT NULL COMMENT '喂丝速度（箱/小时）',
  `material_consumption` json DEFAULT NULL COMMENT '物料消耗信息（JSON）',
  ...
  KEY `idx_planned_time` (`planned_start`,`planned_end`),
  KEY `idx_feeder_code` (`feeder_code`),
  KEY `idx_order_status` (`order_status`),
  KEY `idx_priority` (`priority`),
  KEY `fk_feeding_material` (`article_nr`),
  KEY `idx_feeding_order_composite` (`task_id`,`order_status`,`planned_start`),
...
CREATE TABLE `aps_input_batch` (
  ...
  `input_plan_id` varchar(50) DEFAULT NULL COMMENT '前工序计划号（喂丝机工单号）',
  `input_batch_code` varchar(50) DEFAULT NULL COMMENT '前工序批次号',
  `material_code` varchar(100) NOT NULL COMMENT '物料代码',
  `bom_revision` varchar(50) DEFAULT NULL COMMENT '版本号',
  ...
  `updated_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
  KEY `idx_packing_order` (`packing_order_id`),
  KEY `idx_input_plan` (`input_plan_id`),
  KEY `idx_material_code` (`material_code`),
  KEY `idx_batch_sequence` (`batch_sequence`),
  CONSTRAINT `aps_input_batch_ibfk_1` FOREIGN KEY (`packing_order_id`) REFERENCES `aps_packing_order` (`id`) ON DELETE CASCADE
...
CREATE TABLE `aps_machine` (
  ...
  `equipment_type` varchar(50) DEFAULT NULL COMMENT '设备型号(如PROTOS70, M8)',
  ...
  KEY `idx_status` (`status`)
) ENGINE=InnoDB AUTO_INCREMENT=76 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='机台基础信息表';
...
CREATE TABLE `aps_machine_relation` (
  ...
  KEY `idx_feeder_code` (`feeder_code`),
  KEY `idx_maker_code` (`maker_code`),
  KEY `idx_effective_date` (`effective_from`,`effective_to`),
  KEY `idx_status` (`status`),
  CONSTRAINT `fk_feeder` FOREIGN KEY (`feeder_code`) REFERENCES `aps_machine` (`machine_code`) ON DELETE CASCADE,
  CONSTRAINT `fk_maker` FOREIGN KEY (`maker_code`) REFERENCES `aps_machine` (`machine_code`) ON DELETE CASCADE
...
CREATE TABLE `aps_maintenance_plan` (
  ...
  `maint_group` varchar(50) DEFAULT NULL COMMENT '轮保班组',
  ...
  `estimated_duration` int DEFAULT NULL COMMENT '预计耗时（分钟）',
  `maint_type` varchar(50) DEFAULT NULL COMMENT '轮保类型',
  `maint_level` varchar(50) DEFAULT NULL COMMENT '轮保级别',
  ...
  `mes_version` varchar(50) DEFAULT NULL COMMENT 'MES版本号',
  `planner` varchar(100) DEFAULT NULL COMMENT '制单人',
  `created_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
  ...
  KEY `idx_maint_time` (`maint_start_time`,`maint_end_time`),
  KEY `idx_plan_status` (`plan_status`),
  CONSTRAINT `fk_maint_machine` FOREIGN KEY (`machine_code`) REFERENCES `aps_machine` (`machine_code`) ON DELETE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='设备轮保计划表';
...
CREATE TABLE `aps_mes_dispatch` (
  ...
  `updated_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
  KEY `idx_dispatch_batch` (`dispatch_batch_id`),
  KEY `idx_work_order` (`work_order_nr`),
...
CREATE TABLE `aps_order_status_sync` (
  ...
  `work_order_nr` varchar(50) DEFAULT NULL COMMENT '关联工单号',
...
CREATE TABLE `aps_packing_order` (
  ...
  `work_order_nr` varchar(50) NOT NULL COMMENT '工单号',
  `task_id` varchar(50) NOT NULL COMMENT '排产任务ID',
  `original_order_nr` varchar(50) DEFAULT NULL COMMENT '原始订单号',
  `article_nr` varchar(100) NOT NULL COMMENT '成品烟牌号',
  `quantity_total` int NOT NULL COMMENT '投料总量（箱）',
  `final_quantity` int NOT NULL COMMENT '成品数量（箱）',
  `maker_code` varchar(20) NOT NULL COMMENT '卷包机代码',
  `machine_type` varchar(50) DEFAULT NULL COMMENT '机台型号',
  `planned_start` datetime NOT NULL COMMENT '计划开始时间',
  `planned_end` datetime NOT NULL COMMENT '计划结束时间',
  `estimated_duration` int DEFAULT NULL COMMENT '预计耗时（分钟）',
  `sequence` int NOT NULL COMMENT '执行顺序（同一天内从1开始）',
  ...
  `plan_date` date NOT NULL COMMENT '计划日期（YYYY-MM-DD）',
  `production_speed` int DEFAULT NULL COMMENT '生产速度（支/分钟）',
  `working_shifts` json DEFAULT NULL COMMENT '工作班次信息（JSON）',
  `feeder_code` varchar(20) NOT NULL COMMENT '对应喂丝机代码',
  `related_feeder_order` varchar(50) DEFAULT NULL COMMENT '关联喂丝机工单号',
  `order_status` enum('PLANNED','DISPATCHED','IN_PROGRESS','COMPLETED','CANCELLED') DEFAULT 'PLANNED' COMMENT '工单状态',
  `priority` int DEFAULT '5' COMMENT '优先级（1-10，数值越小优先级越高）',
  `is_split_order` tinyint(1) DEFAULT '0' COMMENT '是否为拆分工单',
  `split_from` varchar(50) DEFAULT NULL COMMENT '拆分来源工单号',
  `split_index` int DEFAULT NULL COMMENT '拆分序号',
  `is_merged_order` tinyint(1) DEFAULT '0' COMMENT '是否为合并工单',
  `merged_from` json DEFAULT NULL COMMENT '合并来源工单列表（JSON）',
  `is_backup_order` tinyint(1) DEFAULT '0' COMMENT '是否为备用工单（对应MES IsBackup字段）',
  `backup_reason` varchar(200) DEFAULT NULL COMMENT '备用原因',
  `processing_history` json DEFAULT NULL COMMENT '处理历史记录（JSON）',
  `created_by` varchar(100) DEFAULT 'system' COMMENT '创建者',
  `created_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  `work_order_type` varchar(10) NOT NULL COMMENT '工单类型',
  ...
  KEY `idx_order_status` (`order_status`),
  KEY `idx_priority` (`priority`),
  KEY `fk_packing_material` (`article_nr`),
  KEY `idx_packing_order_composite` (`task_id`,`order_status`,`planned_start`),
...
CREATE TABLE `aps_processing_log` (
  ...
  CONSTRAINT `fk_log_task` FOREIGN KEY (`task_id`) REFERENCES `aps_scheduling_task` (`task_id`) ON DELETE CASCADE
...
CREATE TABLE `aps_shift_config` (
  ...
  KEY `idx_effective_date` (`effective_from`,`effective_to`),
  KEY `idx_status` (`status`)
) ENGINE=InnoDB AUTO_INCREMENT=3 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='班次配置表';
...
CREATE TABLE `aps_work_order_sequence` (
  ...
  `last_order_nr` varchar(50) DEFAULT NULL COMMENT '最后生成的工单号',
  `created_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
