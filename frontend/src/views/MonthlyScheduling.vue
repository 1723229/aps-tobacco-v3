...
const refreshPlans = async () => {
  ...
  try {
    ...
    const allRecords = (historyResponse.data as any).imports
...
const viewGanttChart = (planOrTask: AvailableBatch | SchedulingTask) => {
  ...
  let monthlyBatchId: string
  
  if ('task_id' in planOrTask) {
    // SchedulingTask (来自历史记录)
    taskId = planOrTask.task_id
    monthlyBatchId = (planOrTask as any).monthly_batch_id || (planOrTask as any).import_batch_id
  } else {
    // AvailableBatch (来自主列表)
    taskId = (planOrTask as any).task_id
    monthlyBatchId = planOrTask.batch_id // 这里就是monthly_batch_id
  }
  
  console.log('🔗 跳转甘特图:', { 
    taskId, 
    monthlyBatchId, 
    planOrTask: planOrTask,
    keys: Object.keys(planOrTask)
  })
  
  // 如果没有月度批次ID，尝试从其他字段获取
  if (!monthlyBatchId) {
    monthlyBatchId = (planOrTask as any).monthly_batch_id || 
                     (planOrTask as any).import_batch_id ||
                     (planOrTask as any).batch_id
  }
  
  if (!monthlyBatchId) {
    ElMessage.error('缺少批次ID信息，无法查看甘特图')
    return
  }
